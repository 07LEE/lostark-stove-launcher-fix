...
`stove-launcher.sh`는 설치 프로그램을 겸합니다. 설치된 자리가 아닌 곳에서 실행되면 설치 모드로, 설치된 자리에서 실행되면 런처 모드로 동작합니다. `.deb`으로 설치되면 `/usr/bin`에 놓이는데, 시스템 경로에서 실행될 때는 설치 모드를 건너뛰고 항상 런처로 동작합니다.
